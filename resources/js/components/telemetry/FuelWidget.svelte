<script lang="ts">
    ...
    // Fuel logic: < 10% critical, < 20% warning
    $: status = fuelLevel <= 10 ? 'critical' : fuelLevel <= 20 ? 'warning' : 'normal';
</script>
...
<WidgetCard title="Уровень топлива" icon="⛽" {status}>
    <div class="flex items-center justify-center w-full relative">
        ...
        <div class="absolute flex flex-col items-center justify-center">
            <span class="text-2xl font-bold font-mono">{Math.round(fuelLevel)}%</span>
        </div>
    </div>
</WidgetCard>
