    ...
    async function fetchData() {
        ...
        try {
            ...
            const res = await fetch(
                `/api/telemetry/analytics?locomotive_id=${$activeLocomotiveId}&start_date=${startDate!.toISOString()}&end_date=${endDate!.toISOString()}${stepParam}`,
            );
            if (res.ok) {
...
{#if isOpen}
    ...
    >
        <div
            class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl w-full max-w-6xl flex flex-col h-[85vh]"
            onclick={(e) => e.stopPropagation()}
        >
            <div
                class="p-4 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center bg-zinc-50 dark:bg-zinc-800/30 rounded-t-xl"
            >
                <h2 class="text-lg font-bold">
                    Архив телеметрии
                    <span
                        class="text-emerald-600 font-mono ml-2 block sm:inline"
                        >{$activeLocomotiveId}</span
                    >
                </h2>
                <div class="flex items-center gap-4 text-sm text-zinc-500">
                    <div>
                        {startDate?.toLocaleString('ru-RU')} &mdash; {endDate?.toLocaleString(
                            'ru-RU',
                        )}
                    </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    >
                        <svg
                            ...
                            /><polyline points="14 2 14 8 20 8" /><path
                                d="M8 13h2"
                            /><path d="M8 17h2" /><path d="M14 13h2" /><path
                                d="M14 17h2"
                            /></svg
                        >
                        CSV
                    ...
                    <button
                        onclick={close}
                        class="p-2 ml-2 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded transition"
                        >&times;</button
                    >
                </div>
            </div>
            ...
            <div
                class="flex-1 p-6 flex flex-col relative bg-zinc-50/50 dark:bg-zinc-900/50 overflow-hidden"
            >
                {#if loading}
                    <div class="flex-1 flex justify-center items-center">
                        <div
                            class="flex flex-col items-center gap-4 text-emerald-600"
                        >
                            <div
                ...
                {:else if !hasData}
                    <div
                        class="flex-1 flex flex-col justify-center items-center text-zinc-500 gap-4"
                    >
                        <svg
                        ...
                        Аналитические данные для выбранного локомотива в данном периоде
                        отсутствуют.
                    </div>
                {/if}
