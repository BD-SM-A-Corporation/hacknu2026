...
<!-- Control Action Bar -->
<div class="flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 shadow-sm mb-6">
    <div class="flex items-center gap-4">
        <div>
            <span class="text-sm opacity-60">Текущий локомотив:</span>
            <div class="text-xl font-bold flex items-center gap-2">
                {#if $activeLocomotiveId}
                    <span class="text-emerald-600 dark:text-emerald-400">{$activeLocomotiveId}</span>
                {:else}
...
{#if $activeLocomotiveId}
    ...
    <div class="mt-6">
        <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 min-h-64 flex items-center justify-center shadow-sm">
            <div class="text-center opacity-50">
                <svg class="mx-auto h-12 w-12 mb-3" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"></polyline></svg>
                <p>Место для графиков динамики (Chart.js / D3)</p>
            </div>
        </div>
    </div>
{:else}
    <div class="flex-1 flex flex-col items-center justify-center p-12 border-2 border-dashed border-zinc-300 dark:border-zinc-700 rounded-xl bg-zinc-50 dark:bg-zinc-800/30">
        <svg class="h-16 w-16 text-zinc-400 mb-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"></path><line x1="4" y1="22" x2="4" y2="15"></line></svg>
        <h2 class="text-xl font-bold mb-2">Ожидание выбора</h2>
        <p class="text-zinc-500 mb-6 text-center max-w-sm">Выберите локомотив из списка для мониторинга телеметрии в реальном времени</p>
        <button
            on:click={openLocomotiveSelector}
